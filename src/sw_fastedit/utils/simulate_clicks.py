import argparse
import os
from convert_json import gc_to_swfastedit_format
...
def save_click_heatmaps_gc(json_file_gc, pet_nii_file, click_save_path, save_nifti=False):
        
    with open(json_file_gc, 'r') as f:
        json_data = json.load(f)
    clicks = gc_to_swfastedit_format(json_data)
    pet_im = nib.load(pet_nii_file)
    ref_shape = pet_im.shape
    ref_affine = pet_im.affine
    ...
    if save_nifti:
        nib.save(tumor_nifti, os.path.join(click_save_path, f'{pet_nii_file.split("/")[-1].split(".nii.gz")[0]}_0003.nii.gz')) # foreground clicks
        nib.save(non_tumor_nifti, os.path.join(click_save_path, f'{pet_nii_file.split("/")[-1].split(".nii.gz")[0]}_0004.nii.gz')) # background clicks
